...
var _ NodeInterface = &TimeTypeNode{}

// TimeTypeNode is a node type that encapsulates fields that are internally
// time based. E.g metav1.ObjectMeta.CreationTimestamp or
// metav1.ObjectMeta.DeletionTimestamp. These are internally of type
// metav1.Time which use standard time type, but their values are specified
// as timestamp strings with parsing logic to create time objects. For
// use-case we only care if the value is set, so we create TimeTypeNodes and
// mark them as leafnodes.
