...
func IgnoreLowerLevelMetaFields(node resourcetree.NodeInterface) bool {
	...
	depth := len(strings.Split(node.GetData().NodePath, "."))
	return !((strings.Contains(lowerCaseFieldName, "objectmeta") || strings.Contains(lowerCaseFieldName, "typemeta")) &&
		depth > 2)
}

// IgnoreRootStatusFields rule ignores Foo.Status for type Foo, as we're only
// interested in coverage of fields that can be set by users (Foo.Spec etc.)
func IgnoreRootStatusFields(node resourcetree.NodeInterface) bool {
	lowerCaseFieldName := strings.ToLower(node.GetData().Field)
	depth := len(strings.Split(node.GetData().NodePath, "."))
	return !(depth == 2 && lowerCaseFieldName == "status")
